# keycloak-release — release automation for the CockroachDB fork
...
The pipeline runs here; only the published branches live in the fork. Both sides
need configuring, and the fork's half is now a single switch.
...
```bash
gh api -X PUT repos/p2-inc/keycloak/actions/permissions -F enabled=false
```

**Here — four repository secrets.**
...
gh secret set ANTHROPIC_API_KEY --repo p2-inc/keycloak-release
```

Repository secrets, not environment secrets, for two reasons. Environment
