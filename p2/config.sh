...
# owner/name of the same repository. The pipeline lives in its own repo now, so
# `github.repository` is this one, not the fork: every read and write aimed at
# the published branches has to name the fork explicitly.
FORK_SLUG=${FORK_SLUG-"p2-inc/keycloak"}
